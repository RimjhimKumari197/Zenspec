...
function updateSlider(value) {
  let percentage = (value / 100000) * 100;
...
function toggleDropdown(Class, id) {
  ...
  const url = new URL(window.location.href);
  if (url.searchParams.has("Filter_key_pref")) {
    url.searchParams.delete("Filter_key_pref");
...
function toggleDropdown2(Class, id) {
  // If there's a selected value, remove it when dropdown is clicked
  const url = new URL(window.location.href);
  if (url.searchParams.has("Filter_prof_tags")) {
    url.searchParams.delete("Filter_prof_tags");
  ...
  const url = new URL(window.location.href);
   ...
   setTimeout(()=>{
    if (url.searchParams.get("sliderMin") || url.searchParams.get("sliderMax")) {
      activeFilter = "slider";
      if(url.searchParams.get("sliderMax")==50000){
